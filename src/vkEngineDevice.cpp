                                      ...
                                      VkDebugUtilsMessengerEXT*                 pDebugMessenger)
{
	if (const auto func = reinterpret_cast <PFN_vkCreateDebugUtilsMessengerEXT>(
		...
		func != nullptr) { return func(instance, pCreateInfo, pAllocator, pDebugMessenger); }
                                   ...
                                   const VkAllocationCallbacks*   pAllocator)
{
	if (const auto func = reinterpret_cast <PFN_vkDestroyDebugUtilsMessengerEXT>(
		...
		func != nullptr) { func(instance, debugMessenger, pAllocator); }
}
...
VkEngineDevice::~VkEngineDevice()
{
	...
	if (enableValidationLayers) { DestroyDebugUtilsMessengerEXT(pInstance, pDebugMessenger, nullptr); }
...
void VkEngineDevice::createInstance()
{
	...
	if (vkCreateInstance(&createInfo, nullptr, &pInstance) != VK_SUCCESS) { throw std::runtime_error("failed to create instance!"); }
...
void VkEngineDevice::pickPhysicalDevice()
{
	...
	if (pPhysicalDevice == VK_NULL_HANDLE) { throw std::runtime_error("failed to find a suitable GPU!"); }
...
void VkEngineDevice::setupDebugMessenger()
{
	...
	if (CreateDebugUtilsMessengerEXT(pInstance, &createInfo, nullptr, &pDebugMessenger) !=
	    VK_SUCCESS) { throw std::runtime_error("failed to set up debug messenger!"); }
}
...
bool VkEngineDevice::checkValidationLayerSupport() const
{
	...
	if (layerCount == 0) { throw std::runtime_error("No layers found"); }
...
bool VkEngineDevice::checkDeviceExtensionSupport(const VkPhysicalDevice device) const
{
	...
	for (uint32_t i = 0; i < extensionCount; ++i) { requiredExtensions.erase(availableExtensions[i].extensionName); }
...
QueueFamilyIndices VkEngineDevice::findQueueFamilies(const VkPhysicalDevice device) const
{
	...
	for (uint32_t i = 0; i < queueFamilyCount; ++i)
	{
		if (queueFamilies[i].queueCount > 0 &&
		    (queueFamilies[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) != 0u) { indices.mGraphicsFamily = i; }
		VkBool32 presentSupport = 0u;
		vkGetPhysicalDeviceSurfaceSupportKHR(device, i, pSurface, &presentSupport);
		if (queueFamilies[i].queueCount > 0 && presentSupport != 0u) { indices.mPresentFamily = i; }
...
{
	if (vkCreateImage(pDevice, &imageInfo, nullptr, &image) != VK_SUCCESS) { throw std::runtime_error("failed to create image!"); }
	...
	if (vkBindImageMemory(pDevice, image, imageMemory, 0) != VK_SUCCESS) { throw std::runtime_error("failed to bind image memory!"); }
}
} // namespace vke